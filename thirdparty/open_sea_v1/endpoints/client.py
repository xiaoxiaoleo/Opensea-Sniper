...
from wconfig import  HTTP_PROXY_URL

from thirdparty.open_sea_v1.helpers.rate_limiter import RateLimiter
from thirdparty.open_sea_v1.responses.abc import BaseResponse

logger = get_logger('opensesniper')
...
class BaseClient(ABC):
    ...
    async def _aget_parsed_pages(self) -> List[List[Type[BaseResponse]]]:
        ...
        async with RateLimiter(rate_limit=self._rate_limit, concurrency_limit=self._concurrency_limit) as rate_limiter:
            async with ClientSession(headers=self.http_headers, json_serialize=ujson.dumps, connector=TCPConnector(verify_ssl=False)) as session:
    ...
    async def _async_get_pages_jsons(self, session, *, rate_limiter: RateLimiter) -> Optional[List[dict]]:
        ...
        while self._remaining_pages():
            ...
            async with rate_limiter.throttle():
                #resp = await session.get(querystring, proxy='http://127.0.0.1:8234')
                if HTTP_PROXY_URL:
                    resp = await session.get(querystring, proxy=HTTP_PROXY_URL)
                else:
