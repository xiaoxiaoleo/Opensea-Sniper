...
from thirdparty.open_sea_v1.helpers.extended_classes import ExtendedStrEnum
from thirdparty.open_sea_v1.responses.event import EventResponse
...
class EventsEndpoint(BaseClient, BaseEndpoint):
    ...
    def _validate_param_occurred_before(self) -> None:
        if not isinstance(self.occurred_before, (type(None), int)):
            raise TypeError('Invalid occurred_before type. Must be instance of int.',
                            f'{type(self.occurred_before)=}')

    def _validate_param_occurred_after(self) -> None:
        if not isinstance(self.occurred_after, (type(None), int)):
            raise TypeError('Invalid occurred_after type. Must be instance of int.',
